...
* and [1, 2, 2, 2, 2] do not. In case of a plateau-peak, please only
...
function pickPeaks(arr){
    //const peakArr = [];
    const peakArr = arr.reduce((res , cur , index, arr) => {
        if ( (arr[index-1] < cur) && (cur > arr[index + 1] ||
             cur > arr[index-1] && cur === arr[index + 1 ] &&
             arr.slice(index).find( item => item !== cur) < cur
        )){
            res['pos'].push(index);
            res['peaks'].push(cur);
        }
