...
function findEvenIndex(arr) {
    ...
    //3. var left = 0, right = arr.reduce(function(pv, cv) { return pv + cv; }, 0);
    ...
    const findEvenIndex = a => a.findIndex((el, i) => sum(a, 0, i) === sum(a, i + 1));
     for (let i = 0; i < arr.length  ; i++ ){
         let startSliceArr = arr.slice( 0, i);
         let endSliceArr = arr.slice( i + 1, arr.length);
         let sumOfStart = startSliceArr.length === 0 ? 0 : startSliceArr.reduce((item , sum ) => ( sum += item ) );
         let sumOfEnd  = endSliceArr.length === 0 ? 0 : endSliceArr.reduce((item , sum ) => ( sum += item ) );
...
//console.log(findEvenIndex([20,10,30,10,10,15,35])); //0
